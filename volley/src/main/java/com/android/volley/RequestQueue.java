...
public class RequestQueue {
    ...
    public void start() {
        ...
        //创建了一个CacheDispatcher的实例，然后调用了它的start()方法(CacheDispatcher跟NetworkDispatcher都持有mNetworkQueue的引用)
        ...
        for (int i = 0; i < mDispatchers.length; i++) {
            //for循环里去创建NetworkDispatcher的实例，并分别调用它们的start()方法
...
//            不能缓存则在第12行直接将这条请求加入网络请求队列
            mNetworkQueue.add(request);
            return request;
        }
//        可以缓存的话则在第33行将这条请求加入缓存队列（默认情况下，每条请求都是可以缓存的）
    ...
    <T> void finish(Request<T> request) {
        ...
        //Set<Request<?>>：所有被添加的Request都进入这一集合，这样在RequestQueue#cancelAll时，就是取消这个集合当中的队列。
        synchronized (mCurrentRequests) {
