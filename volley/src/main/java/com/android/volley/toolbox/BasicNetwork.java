...
//                调用了AdaptedHttpStack的executeRequest方法(把Request和Header传给HttpStack)
                httpResponse = mBaseHttpStack.executeRequest(request, additionalRequestHeaders);
                //HttpStack发起请求之后，获得HttpResponse的StatusCode和Header
                int statusCode = httpResponse.getStatusCode();

                responseHeaders = httpResponse.getHeaders();
                // Handle cache validation.
                //根据StatusCode来构建NetworkResponse，判断是否是304：
                if (statusCode == HttpURLConnection.HTTP_NOT_MODIFIED) {
                   // 再判断mCacheEntry是否为空，如果为空，那么仅仅传入头部构建
                    ...
                    //否则更新mCacheEntry的头部，传入mCacheEntry.data来构建
                    return new NetworkResponse(
                            HttpURLConnection.HTTP_NOT_MODIFIED,
                            entry.data,
                            /* notModified= */ true,
                ...
                //从Entity中获得byte[]构建NetworkResponse。
                return new NetworkResponse(
                        statusCode,
                        responseContents,
                        /* notModified= */ false,
    ...
    private static void attemptRetryOnException(
        ...
        try {
            //其实是修改当前Request对应的RetryPolicy中的超时时间，这样再下次请求时，该Request所允许超时的时间就会变长，从而减少超时情况的发生
