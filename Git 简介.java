...
重点
git reset commit （commit可以为分支名（表示该分支的最后一个提交),可以为一次具体的提交，可以用HEAD~表示),既会移动HEAD,也会让该HEAD指向的分支一起移动
git checkout branch 只会移动HEAD(与git reset branch相对应，如果只是git checkout commit 会导致detached HEAD，需要git checkout -b detach_test创建一个分支，该分支的最后提交为commit，并切换到该分支)
重点
git reset commit file.txt（像这种带后缀路径的，不会移动HEAD，也不会移动分支）
git checkout commit file.txt（与git reset 带路径一样）
...
不带file:
git reset [commit] 都会移动HEAD
区别：set 会移动 HEAD 分支的指向，而 checkout 则移动 HEAD 自身.例如当前为develop分支，调用git reset master，
develop的HEAD节点会移到master指向的最后一次commit中。
重点:（如果只是git checkout commit 会导致detached HEAD，需要git checkout -b detach_test创建一个分支，该分支的最后提交为commit，并切换到该分支)


带file:(reset的commit省的话用HEAD,checkout的commit省的话意思用暂存区覆盖工作区（git checkout -- file.txt 与git checkout file.txt效果一样）,且git rest --soft commit file.txt 是无效的)
git reset (commit) [file] 和git checkout (commit) [file] 都不会移动HEAD
 区别:git reset (commit) [file] 等同于git reset --mixted(commit) [file],有使用指定commit的file覆盖暂存区的效果（这时有点效果就是，工作区还是最新的，
 暂存区是检出来的那次commit的文件，所以如果这时没改东西，取消暂存或者把对应file加入暂存，都会变没,可以把工作区的丢弃掉），而
git checkout (commit) [file]等同于git reset --hard(commit) [file] ，不仅用某次提交中的那个文件来更新索引，同时也会覆盖工作目录中对应的文件 —— 这样对工作目录并不安全！
注意：一旦暂存了，想恢复到工作区，只能用 git reset HEAD <file>(此时使用上一次提交的历史记录还原暂存区，工作区不变，因为暂存区是上次提交的状态，所以此时工作区的状态是，
	有修改，未暂存)，不想恢复到工作区，直接还原到上次暂存区状态，用git checkout HEAD <file>(等同git reset --hard(commit) [file] )；没暂存，可以用git checkout -- <file>清除工作区的修改
https://blog.csdn.net/longintchar/article/details/82314102
