...
public class DrawableTypeRequest<ModelType> extends DrawableRequestBuilder<ModelType>
    ...
    private static <A, Z, R> FixedLoadProvider<A, ImageVideoWrapper, Z, R> buildProvider(
        ...
        //A:String  resourceClass&Z:GifBitmapWrapper R:GlideDrawable
...
//	    调用了glide.buildTranscoder()方法来构建一个ResourceTranscoder，它是用于对图片进行转码的
    ...
    DrawableTypeRequest(Class<ModelType> modelClass,
        ...
        super(context, modelClass,
                //buildProvider返回的FixedLoadProvider包括modelLoader, transcoder（转码）,
                //                 dataLoadProvider（编解码），modelLoader其实是ImageVideoModelLoader
                //                 （StreamStringLoader再封装了一层）
...
//        创建了GifTypeRequest
        return optionsApplier
                .apply(new GifTypeRequest<ModelType>(this, streamModelLoader, optionsApplier));
    }
    ...
    private GenericTranscodeRequest<ModelType, InputStream, File> getDownloadOnlyRequest() {
        return optionsApplier
                .apply(new GenericTranscodeRequest<ModelType, InputStream, File>(File.class, this,
